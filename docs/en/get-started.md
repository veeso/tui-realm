...
- **AppComponent**: An AppComponent is a wrapper around a **Component** which represents a single component in your application. It directly takes events and generates messages for the application consumer. Underneath it relies on its Component for properties/states management and rendering.
...
Additionally, keep in mind that **AppComponent** is always a **Component**, but a **Component** only *may* be an **AppComponent**.
...
So, apparently the Component defines everything we need handle properties, states and rendering. So why are we not done yet and we need an AppComponent trait too?

1. Component must be agnostic over **Event**s: Components may be distribuited in libraries (e.g. `tui-realm-stdlib`) and because of that, they cannot consume `Event` or produce `Message`.
...
Let's see what an AppComponent is in practice now:
...
Quite simple eh? Yep, it was my intention to make them as light as possible, since you'll have to implement one for each component in your view. As you can also notice, an **AppComponent** requires to also be a **Component** so in practice we'll also have something like:
...
As you can see, the **AppComponent** consumes an `Event` and produces a `Msg`, while the Component, which is called by the AppComponent, consumes a `Cmd` and produces a `CmdResult`.
...
flowchart TD
    ...
    subgraph comp [AppComponent]
        AppComp["AppComponent::on"]
        BaseComp["Component::perform"]
    end
    ...
    %% AppComponent links
    AppComp -->|Cmd| BaseComp
    BaseComp -->|CmdResult| AppComp
    AppComp ==>|Msg| FocusedRet & SubscriptionsRet
```
...
With that our **Component** is ready, we could now implement our two **AppComponents**, but we also need some other types like **Messages**, so lets go and do those first.

### Let's define the message type

Before implementing the two `AppComponent` we first need to define the messages our application will handle.
...
First we define the `LetterCounter` with the Component within. Since we don't need any particular behaviour for the `Component` trait, we can simply derive `Component`, which will automatically pass through all `Component` calls to the underlying `Component`. If you want to read more read see [tuirealm_derive](https://github.com/veeso/tui-realm/tree/main/crates/tuirealm_derive).

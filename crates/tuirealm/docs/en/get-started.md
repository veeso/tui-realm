...
- [Get Started 🏁](#get-started-)
  ...
  - [Component Vs. AppComponent](#component-vs-appcomponent)
    - [The Component](#the-component)
    - [The AppComponent](#the-appcomponent)
...
- **Component**: A Component represents a re-usable UI component, which can have some **properties** for rendering or to handle commands. It can also have its own **states**, if necessary. In practice it is a trait which exposes some methods to render and to handle properties, states and events. We'll see it in detail in the next chapter.
- **AppComponent**: A AppComponent is a wrapper around a **Component** which represents a single component in your application. It directly takes events and generates messages for the application consumer. Underneath it relies on its Component for properties/states management and rendering.
...
- **Message** (or usually called `Msg`): A message is a Logic event that is generated by the AppComponents in reaction to a `Event`, to be consumed by the `Model`.
...
- **Command** (or usually called `Cmd`): Is an entity generated by the **AppComponent** when it receives an **Event**. It is used by the AppComponent to operate on its **Component**.
- **View**: The view is where all the components are stored. The view has basically three tasks:
  - **Managing AppComponents mounting/unmounting**: AppComponents are mounted into the view when they're created. The view prevents mounting duplicated AppComponents (on the same `Id`) and will warn you when you try to operate on unmounted AppComponent.
  - **Managing focus**: the view guarantees that only one AppComponent at a time is active. The active AppComponent is enabled with a dedicated attribute (we'll see that later) and all the events will be forwarded to it (and all *Subscribed* AppComponents). The view keeps track of all the previously focused AppComponents, so if the current active AppComponent loses focus, the previous active one is active if there's no other AppComponent to active.
  - **Providing an API to operate on AppComponents**: Once AppComponents are mounted into the view, they must be accessible to the outside, but in a safe way. That's possible thanks to the bridge methods the view exposes. Since each AppComponent must be uniquely identified to be accessed, you'll have to define some **ID**s for your AppComponents.
- **Model**: The model is a structure you'll define for your application to implement the **Update routine**.
- **Update routine**: The update routine is a function, which must be implemented by the **Model**. This function is as simple as it is important. It only takes 2 parameters: itself mutably and the **Message** to process. Based on the *Message*, it invokes a certain behaviour on the Model or on the view. The routine can return a *Message* aswell, which will cause the routine to be called again by the application with the given *Message*. Later, when we'll see the example you'll see how this is just cool.
- **Subscription** (or usually called *Sub*): A subscription is a ruleset which tells the **application** to also forward specific events to other AppComponents even if they're not focused. We'll talk about subscription in advanced concepts.
- **Port**: A port is an event listener which will use a trait called `Poll` to fetch for incoming events. A port defines both the trait to call and an interval which must elapse between each call. The events are then forwarded to the subscribed AppComponents. The input listener is a port, but you may also implement for example an HTTP client, which fetches for some data. Ports are explained in advanced concepts.
...
Separate traits also make for a good separation of concerns and allow modifying how things are passed to the underlying Component.

Additionally, keep in mind that **AppComponent** is always a **Component**, but a **Component** only *may* be a **AppComponent**.

Let's see their definition in details:

### The Component

The component is meant to be agnostic regarding events and *re-usable* in other places (even different projects).

For instance:

- ✅ A Label which shows a single line of text makes a good component.
- ✅ An Input component like `<input>` in HTML is a good component. Even if it can handle many input types, it still has one responsibility, is generic and is re-usable.
- ❌ An input which can handle both text, radio buttons and checks is a bad component. It is too generic.
- ❌ An input which takes the remote address for a server is a bad component. It is not generic.

These are only guidelines, but just to give you the idea of what a component is.

A component also handles **States** and **Props**, which are totally user-defined based on your needs. Sometimes you may even have component which don't handle any state (e.g. a label).

In practice a component is a trait, with these methods to be implmented:
...
- *perform*: Performs the provided **command** on the component. This method is called by the **AppComponent** as we'll see later. The command should change the component states. Once the action has been processed, it must return the `CmdResult` to the **AppComponent**.

### The AppComponent

So, apparently the Component defines everything we need handle properties, states and rendering. So why are we not done yet and we need a component trait too?

1. AppComponent must be agnostic over **Event**s: Components may be distribuited in libraries (e.g. `tui-realm-stdlib`) and because of that, they cannot consume `Event` or produce `Message`.
2. Because of point 1, we need an entity which consumed **Event**s and produces **Messages**. These two entities are totally or partially user-defined, which means, they are different for each realm application. This means the AppComponent must fit to the application.
3. **It's impossible to fit a component to everybody's needs**: I tried to in `tui-realm` 0.x, but it was just impossible. At a certain point I just started to add properties among other properties, but eventually I ended up re-implementing stdlib components from scratch just to have some different logics. Components are good because they're generic, but not too much; they must behave as dummies to us. Components are exactly what we want for the application. For example, we have a Input, and want to change its colors based on what is written; one application may want to change color everytime when `a` is types, others may want to `the` is written. This may be possible with a lot of options, or validation in a given Input, but this logic is better suited to **AppComponent** instead of *Component**. Oh, and I almost forgot the worst thing about generalizing components: **keybindings**.

With that said, what actually is a AppComponent?

A AppComponent is an application specific unique implementation of a component. Let's think for example of a form and let's say the first field is an input text which takes the username. If we think about it in HTML, it will be for sure a `<input type="text" />` right? With just that definition, the input is generic over what text is input, it may have some validation associated, but ultimately it has no meaning on submit. But we want it to be our username input and on Submit generate the appropriate **Message**. So the input text will be the `Component` in tui-realm. But *that* "username" input field, will be *your* **username input text**. The `UsernameInput` *AppComponent* will wrap a `Input` *Component*, but based on incoming events it will operate differently on the component and will produce different **Messages** if compared for instance to a `EmailInput`.

So, let me state the most important thing you must keep in mind from now on: **AppComponents are unique ❗** to your application. You **should never blindly use the same AppComponent more than once**.

Let's see what a AppComponent is in practice now:
...
Quite simple eh? Yep, it was my intention to make them as light as possible, since you'll have to implement one for each component in your view. As you can also notice, a **AppComponent** requires to also be a **Component** so in practice we'll also have something like:

```rust
pub struct UsernameInput {
    component: Input, // Where input implements `Component`
}
...
Due to requiring **Component** to be implemented for each **AppComponent** and it being quite common to pass-through the calls, `#[derive(Component)]` can be used instead.

Another thing you may have noticed and that may frighten some of you are the two generic types that AppComponent takes.
...
Going forward using "**Component**" may imply also having **AppComponent** implemented, unless otherwise specified.
...
If we give a look to the **AppComponent** trait, we'll see that the method `on()` has the following signature:

```rust
fn on(&mut self, ev: &Event<UserEvent>) -> Option<Msg>;
```

and we know that the `AppComponent::on()` will call the `perform()` method of its **Component**, in order to update its states. The perform method has this signature instead:

```rust
fn perform(&mut self, cmd: Cmd) -> CmdResult;
```

As you can see, the **AppComponent** consumes an `Event` and produces a `Msg`, while the underlying component, consumes a `Cmd` and produces a `CmdResult`.
...
The view is basically a box for all the (App)Components. All the components which are part of the same "view" (in terms of UI) must be *mounted* in the same **View**.
...
Once a component is mounted, it won't be directly accessable anymore. The view will store it as a generic `AppComponent` and will expose a bridge to operate on all the components in the view, querying them with their **Identifier**.
...
Whenever you interact with components in a UI, there must always be a way to determine which component will handle the interaction. If I press a key, the View must be able to correctly forward those **Event**s to the focused **AppComponent**.
...
So we've said we have two Counters, one tracking alphabetic characters and one digits, so we've found a potential **Component**: the **Counter**. The counter will just have a state keeping track of "times" as a number and will increment each time a certain command will be sent.
...
Then, we'll implement easy-to-use Builder methods for our Component:
...
Finally we can implement `Component` for `Counter`
...
Before implementing the two `Component`s we first need to define the messages our application will handle.
...
First we define the `LetterCounter` with the Component within. Since we don't need any particular behaviour for the `Component` trait, we can simply derive `Component`, which will automatically pass through all `Component` calls to the underlying `Component`. If you want to read more read see [tuirealm_derive](../../../tuirealm-derive/).
...
Then we implement the constructor for the letter counter, it accepts the initial value and construct a `Counter` using the component constructor:
...
Finally we implement the `AppComponent` trait for the `LetterCounter`. Here we first convert the incoming `Event` to a `Cmd`, then we call `perform()` on the Component to get the `CmdResult` in order to produce a `Msg`.
...
Finally we can implement the `Update` function:
